...
def test_all_products_are_dated(mocked_date: Any,
...
@mock.patch("datetime.date")
def test_outdated_duck(mocked_date: Any, products_list: list[dict]) -> None:
    mocked_date.today.return_value = date(2022, 2, 3)
    assert outdated_products(products_list) == ["duck"]


@mock.patch("datetime.date")
def test_outdated_duck_chicken(mocked_date: Any,
                               products_list: list[dict]) -> None:
...
@mock.patch("datetime.date")
def test_outdated_all(mocked_date: Any, products_list: list[dict]) -> None:
